...
/***/ function(module, exports, __webpack_require__) {
	...
	var _configureNetflixHandler = __webpack_require__(50);

	var _waitForVideoReady = __webpack_require__(51);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	window.addEventListener('load', function () {
	  (0, _waitForVideoReady.waitForVideoReady)().then(function (video) {
	    console.log('[NRC] 🤖  > Waiting for server...');
	    var socket = _socket2.default.connect(_config.config.v1.server);
	    socket.on('connect', (0, _configureNetflixHandler.configureNetflixHandler)(socket, video));
	  });
	});
...
/***/ function(module, exports) {
	...
	var config = exports.config = {
	  v1: {
	    server: 'http://antoinechalifour.fr:9999/v1/extension'
	  }
	};
...
/***/ function(module, exports) {
	...
	var configureNetflixHandler = exports.configureNetflixHandler = function configureNetflixHandler(socket, video) {
	  return function () {
	    console.log('[NRC] 🤖  > Connected. Have fun!');
	    var VOLUME_CHANGE_STEP = 0.15;
	    ...
	    socket.on('vol:mute', function () {
	      lastVolume = $video.volume;
	      $video.volume = 0;
	    });
	    socket.on('vol:unmute', function () {
	      if (lastVolume !== 0 && !lastVolume) {
	        $video.volume = lastVolume;
	      }
	    });
	    socket.on('vol:up', function () {
	      return $video.volume = $video.volume + VOLUME_CHANGE_STEP > 1 ? 1 : $video.volume + VOLUME_CHANGE_STEP;
	    });
	    socket.on('vol:down', function () {
	      return $video.volume = $video.volume - VOLUME_CHANGE_STEP < 0 ? 0 : $video.volume = VOLUME_CHANGE_STEP;
	    });

	    var name = video.name;
	    var season = video.season;
	    var episode = video.episode;

	    socket.emit('video:new', {
	      video: { name: name, season: season, episode: episode }
	    });
	  };
	};
...
/***/ function(module, exports) {
	...
	var waitForVideoReady = exports.waitForVideoReady = function waitForVideoReady() {
	  ...
	  return new Promise(function (resolve, reject) {
	    ...
	    var detectVideo = function detectVideo() {
